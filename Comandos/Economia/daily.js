...
module.exports =  {
    ...
    run: async (client, interaction, args) => {
        ...
        const tru = await db.get(`emp_${interaction.user.id}`)
        ...
        let quantia2 = Math.ceil(Math.random()* 5000); // Máximo de moedas
        if (quantia2 < 1000) quantia2 = quantia2 + 1000; 
        await db.add(`saldoemp_${interaction.user.id}`, quantia)

        let embed = new Discord.EmbedBuilder()
        .setColor('Green')
        .setTitle(`:moneybag:  | Daily Resgatado!`)
        .setDescription(`### Você resgatou sua recompensa diária e recebeu ${quantia} pizzas\n* Sua empresa também Recebeu ${quantia}`)
